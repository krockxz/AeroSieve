...
mod tests {
    ...
    fn temp_config() -> PipelineConfig {
        let base = std::env::temp_dir().join(format!("aerosieve-test-{}", Uuid::new_v4().to_string().split('-').next().unwrap()));
        let rules_path = PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/../aerosieve-lexical/rules/default.yaml"));
        PipelineConfig {
            ring_capacity: 64,
    ...
    fn test_pipeline_sine_wave_passes() {
        ...
        pipeline.push_chunk(SourceKind::Synthetic, audio, "hello world".into()).unwrap();
    ...
    fn test_pipeline_silence_rejected() {
        ...
        pipeline.push_chunk(SourceKind::Synthetic, silence, "nothing".into()).unwrap();
    ...
    fn test_pipeline_normalizes_text() {
        ...
        let audio: Vec<f32> = (0..320).map(|i| (i as f32 * 0.1).sin() * 0.5).collect();
        pipeline.push_chunk(SourceKind::Synthetic, audio, "yeh ₹500 hai".into()).unwrap();
